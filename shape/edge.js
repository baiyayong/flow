...
export default function (G6) {
  G6.registerEdge('flow-polyline-round', {
    ...
    combineBBoxes(sBBox, tBBox) {
      ...
      const maxX = Math.max(sBBox.maxX, tBBox.maxX); const
    ...
    getBBoxFromVertexes(sPoint, tPoint) {
      ...
      const minY = Math.min(sPoint.y, tPoint.y); const
    ...
    getPointBBox(t) {
      return {
        centerX: t.x, centerY: t.y, minX: t.x, minY: t.y, maxX: t.x, maxY: t.y, height: 0, width: 0,
      };
    },
    fillId(points) {
      points.forEach((p) => {

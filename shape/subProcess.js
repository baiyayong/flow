  ...
  G6.registerNode('sub-process-node', {
    ...
    drawControlPoints(group) {
      ...
      this.getControlPoints().forEach((p, i) => {
        ...
        group.getAllAnchors = () => group.controlPointShapes;
        group.getAnchor = j => group.controlPointShapes.filter(a => a.get('index') === j);
      });
    },
    drawNodes(cfg, group) {
      const nodes = [];
    ...
    drawShape(cfg, group) {
      ...
      group.addNodeModel = (_node, _cfg) => this._addNode(_node, _cfg);
      group.addEdgeModel = (_node, _cfg) => this._addEdge(_node, _cfg);
      group.updateNodeModel = (_node, _itemId, _cfg) => this._updateNode(_node, _itemId, _cfg);
      group.getItem = (_node, _itemId) => this._getItem(_node, _itemId);
      group.removeItem = (_node, _itemId) => this._removeItem(_node, _itemId);
